...
return message.lineReply(`> ${Settings.emojiler.iptal} Bu komutu kullanabilmek için sunucuya **Boost** basman  gerekiyor.`).then(x => x.delete({timeout: 7500}));

var isim = args.slice(0).join(" ");

if(!isim) return message.lineReply(`> ${Settings.emojiler.iptal} Değiştirilecek bir nick yazmalısın.(.rich @user/ID)`);
