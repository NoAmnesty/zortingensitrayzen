...
if(!args[0]) return message.lineReply(embed.setDescription(`${Settings.emojiler.iptal} Geçerli bir kullanıcı giriniz.`))
await client.users.fetch(args[0]).then(res => {
    if(!res){
        embed.setDescription(`${Settings.emojiler.iptal} Geçerli bir kullanıcı giriniz.`)
        return message.lineReply(embed)
    }else{
        message.guild.fetchBans(true).then(async(bans) => {
            let ban = await bans.find(a => a.user.id === res.id)
            if(!ban){
                embed.setDescription(`${Settings.emojiler.iptal} \`${res.tag}\` Bu kullanıcı banlanmamış değil.`)
                return message.lineReply(embed)
            }else{
                let text = `${Settings.emojiler.ban} **${res.tag}** (\`${res.id}\`) Adlı üyenin yasaklanma bilgileri: \n\n\`\`\`"${ban.reason || "Sebep Yok"}"\`\`\``
                message.guild.fetchAuditLogs({type: 'MEMBER_BAN_ADD', limit: 100}).then(audit => {
                    let user = audit.entries.find(a => a.target.id === res.id)
                    if(user){
                        embed.setDescription(text + `\n\n\`•\` Yasaklayan Yetkili: \`${user.executor.tag}\` (\`${user.executor.id}\`)\n\`•\` Yasaklanma Tarihi: \`${moment(user.createdAt).format("lll")}\``)
                        return message.lineReply(embed)
                    }else{
